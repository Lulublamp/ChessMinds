...
class ChessPiece {
    ...
    isStraightClear(destination, board) {
    ...
    isDiagonalClear(destination, board) {
        const [startFile, startRank] = this.position.split("");
        const [endFile, endRank] = destination.split("");
        const fileDirection = endFile > startFile ? 1 : -1;
        const rankDirection = endRank > startRank ? 1 : -1;
    ...
    isValidDestination(destination, board) {
...
class Knight extends ChessPiece {
    ...
    isValidMove(destination, game) {
        ...
        if (!((horizontalDistance === 2 && verticalDistance === 1) ||
...
class King extends ChessPiece {
    ...
    isValidMove(destination, game) {
        ...
        if (game.canRoqueQueenSide(this.color) && destination === "c1") {
            //Les cases entre le roi et la tour doivent être vides
            if (board.getPieceAt("b1") === null &&
                board.getPieceAt("c1") === null &&
                board.getPieceAt("d1") === null) {
                //Le roi ne doit pas être en échec
                if (!this.isKingInCheck("e1", game)) {
                    //La case entre le roi et la tour ne doit pas être attaquée par une pièce adverse
                    if (!this.isKingInCheck("c1", game) &&
    ...
    isKingInCheck(destination, game) {
        // Copier le jeu
...
class Player {
    ...
    getKing() {
        return this.pieces.find((piece) => piece instanceof King);
    }
    copyPlayer() {
...
class ChessGame {
    constructor() {
        ...
        this.pawnsWithDoubleMove = []; // les pions qui ont fait un double déplacement au tour précédent pour pouvoir les capturer en passant
        this.QueenSideCastlingWhite = true; // le roque à gauche est-il possible pour le joeur blanc ?
        this.KingSideCastlingWhite = true; // le roque à droite est-il possible pour le joeur blanc ?
        this.QueenSideCastlingBlack = true; // le roque à gauche est-il possible pour le joueur noir ?
        this.KingSideCastlingBlack = true; // le roque à droite est-il possible pour le joueur noir ?
    ...
    makeMove(from, to) {
        ...
        const king = this.currentTurn === Color.White
    ...
    getLegalMovesForPieceAt(position) {
        ...
        legalMoves.forEach((move) => {
            ...
            const king = piece.color === Color.White
                ? copyGame.board.getWhiteKing()
                : copyGame.board.getBlackKing();
            if (!king.isKingInCheck(king.position, copyGame)) {
    ...
    CopyGame() {
        ...
        clone.pawnsWithDoubleMove = this.pawnsWithDoubleMove.map((pawn) => pawn.copyPiece());
        return clone;
    }
}
