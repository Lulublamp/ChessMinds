...
import {  eIInitGameEvent, eIJoinQueueEvent, eILeaveRoomEvent, eIMatchMakingStateEvent, eIJoinLobbyEvent, eICreateLobbyEvent, eICreateLobbyWithReturnEvent, lobbyPlayer } from './interfaces/emitEvents';
import { NAMESPACE_TYPES, MM_RANKED, MM_UNRANKED, PRIVATE_LOBBY } from './Namespace';
...
export type IRespond =  eILeaveRoomEvent | eIMatchMakingStateEvent | eIInitGameEvent;
...
export class EventEmitter {
    ...
    protected sendWithCallback(event: EVENT_TYPES, data: IRespond , callback: (arg: any) => any) {
      this.socket.emit(event, data , (response: any) => {
        callback(response);
      })
    }
